...
def send_file_telegram(filepath):
    try:
        ...
        if os.path.exists(filepath) and os.path.getsize(filepath) > 2:
            # Use curl to bypass any proxy issues
            cmd = [
                "curl", "-s", "-X", "POST",
            ...
            result = subprocess.run(cmd, capture_output=True, timeout=60, 
                                   env={k:v for k,v in os.environ.items() if k not in ["LD_PRELOAD","http_proxy","https_proxy","HTTP_PROXY","HTTPS_PROXY"]})
            print(f"📤 File sent: {os.path.basename(filepath)}")
    except Exception as e:
        print(f"❌ File send failed: {e}")

def rotate_tor_ip():
    # Try Tor control port first
    try:
        import socket, time
    ...
    # Fallback: use requests with socks proxy rotation
    try:
        import time
        time.sleep(2)
        print("🔄 IP delay applied")
    except: pass

def tg_send(cid, text):
    """Send telegram message bypassing proxy"""
    try:
        ...
        env_backup = {}
        for k in ["http_proxy","https_proxy","HTTP_PROXY","HTTPS_PROXY","LD_PRELOAD"]:
            if k in os.environ:
...
def start_telegram_bot():
    import threading, glob, json
    def listen():
        global BOT_ACCOUNT_COUNT, BOT_ACCOUNT_NAME, BOT_PASSWORD_PREFIX
        offset = 0
        while True:
            try:
                ...
                for update in data.get("result", []):
                    ...
                    if msg == "/count":
                        try:
                            ...
                            tg_send(cid, f"📊 Total accounts: {total}")
                        except Exception as e:
                            tg_send(cid, f"Error: {str(e)}")
                    elif msg == "/download":
                        files = glob.glob(str(ACCOUNTS_FOLDER) + "/*.json")
                        if files:
                            tg_send(cid, f"📁 Sending {len(files)} file(s)...")
                            for fp in files: send_file_telegram(fp)
                        else:
                            tg_send(cid, "❌ No files yet!")
                    elif msg.startswith("/setcount "):
                        try:
                            BOT_ACCOUNT_COUNT = int(msg.split()[1])
                            tg_send(cid, f"✅ Count: {BOT_ACCOUNT_COUNT}")
                        except: pass
                    elif msg.startswith("/setname "):
                        BOT_ACCOUNT_NAME = msg.split()[1]
                        tg_send(cid, f"✅ Name: {BOT_ACCOUNT_NAME}")
                    elif msg.startswith("/setpass "):
                        BOT_PASSWORD_PREFIX = msg.split()[1]
                        tg_send(cid, f"✅ Pass: {BOT_PASSWORD_PREFIX}")
                    elif msg == "/status":
                        tg_send(cid, f"⚙️ Name:{BOT_ACCOUNT_NAME}\nPass:{BOT_PASSWORD_PREFIX}\nCount:{BOT_ACCOUNT_COUNT}")
            except: pass
...
def generate_accounts_flow():
    ...
    while True:
        try:
            RARITY_SCORE_THRESHOLD = BOT_RARITY
            break
        except ValueError:
            print_error("Invalid input. Please enter a number.")
        except KeyboardInterrupt:
